...
class Game extends React.Component {
  ...
  handleTouchMove(evt) {
    ...
    if (
      ...
      (Math.abs(xDiff) * 3 > Math.abs(yDiff) &&
        Math.abs(yDiff) * 3 > Math.abs(xDiff))
    ) {
      //ignore
      return;
    }
  ...
  handleAction(action) {
    ...
    document.getElementById("you-score").style.backgroundColor =
      playerColor[myBest];
...
function update_layout() {
  ...
  subTitle.style.marginTop =
    Math.round((h - canvasSize) / 2 - canvasSize * 0.23) + "px";

  const youText = document.getElementById("you-text");
  youText.style.fontSize = Math.round((30 * canvasSize) / 500) + "px";
  youText.style.marginLeft =
    Math.round((w - canvasSize) / 2 + canvasSize * 0.507) + "px";
  youText.style.marginTop =
    Math.round((h - canvasSize) / 2 - canvasSize * 0.35) + "px";

  const aiText = document.getElementById("ai-text");
  aiText.style.fontSize = Math.round((30 * canvasSize) / 500) + "px";
  aiText.style.marginLeft =
    Math.round((w - canvasSize) / 2 + canvasSize * 0.85) + "px";
  aiText.style.marginTop =
    Math.round((h - canvasSize) / 2 - canvasSize * 0.35) + "px";

  const youScore = document.getElementById("you-score");
  youScore.style.fontSize = Math.round((30 * canvasSize) / 500) + "px";
  youScore.style.marginLeft =
    Math.round((w - canvasSize) / 2 + canvasSize * 0.445) + "px";
  youScore.style.marginTop =
    Math.round((h - canvasSize) / 2 - canvasSize * 0.25) + "px";
  youScore.style.width = Math.round(canvasSize * 0.23) + "px";

  const aiScore = document.getElementById("ai-score");
  aiScore.style.fontSize = Math.round((30 * canvasSize) / 500) + "px";
  aiScore.style.marginLeft =
    Math.round((w - canvasSize) / 2 + canvasSize * 0.76) + "px";
  aiScore.style.marginTop =
    Math.round((h - canvasSize) / 2 - canvasSize * 0.25) + "px";
  aiScore.style.width = Math.round(canvasSize * 0.23) + "px";

  const gameInfo = document.getElementById("game-info");
  gameInfo.style.fontSize = Math.round((40 * canvasSize) / 500) + "px";
  gameInfo.style.marginLeft =
    Math.round((w - canvasSize) / 2 + canvasSize * 0.3) + "px";
